# Blackbox Protocol - Vision & Philosophy
...
We love XState. But it wasn't built for our use case.

| Aspect | XState | Blackbox |
|--------|--------|----------|
...
| **Learning Curve** | Steep (actors, spawn) | Gentle (3 primitives) |
...
Use XState under the hood if you want (we did in v0.1).  
But developers never see it.
...
```jsx
{actions.map(action => (
  <button onClick={() => session.do(action)}>
    {action.label}
  </button>
))}
```
...
- Start shipping programs
- Let UIs discover

**For Product Teams**:
- Stop mapping user flows in Figma
- Start writing workflows in JSON
- Let the protocol enforce

**For Architects**:
- Stop coupling frontend to backend
- Start thinking protocol-first
- Let implementation vary
...
*"We are done guessing how to use systems. We just ask the blackbox what's possible right now."*
